...
@click.option('-c', '--config', type=click.Path(dir_okay=False), default='tf_train_config.ini', callback=configure,
              ...
              show_default=True, )
# the data paths
@click.option('--img_dir', type=click.Path(exists=True, file_okay=False), default="./img",
              help="The path to the directory containing the images.")
@click.option('--seg_dir', type=click.Path(exists=True, file_okay=False), default="./seg",
              help="The path to the directory containing the segmentations.")
@click.option('--wei_dir', type=click.Path(exists=True, file_okay=False), default="./wei",
...
@click.option('--image_size', type=(int, int, int), default=(256, 32, 1),
              help='The target image size including channel dimension')
# train specific params
@click.option('--batch_size', type=int, default=32, help='Batch size used for the training.')
@click.option('--epochs', type=int, default=5, help='Number of epochs used for the training.')
...
def main(**kwargs):
    ...
    tf_pipe = TFPipeMotherMachine(img_dir=kwargs["img_dir"], seg_dir=kwargs["seg_dir"], weight_dir=kwargs["wei_dir"],
                                  test_size=kwargs["test_size"], val_size=kwargs["val_size"], loglevel=kwargs["loglevel"],
                                  np_random_seed=kwargs["np_random_seed"], batch_size=kwargs["batch_size"],
