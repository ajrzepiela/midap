    ...
    if len(filename) > 0:
        ...
        if filename is None:
            raise click.BadParameter(
                "Please specify at least one filename as argument or in config file"
            )
        # return the extraced filenames
        filename = re.findall('"(.*)"', filename)
        if len(filename) == 0:
            raise click.BadParameter(
                "No valid filename found in the config file, please make sure each file name "
                'is surrounded by quotes, e.g.  "img_1_raw.tif"'
            )
        return filename
...
@click.option(
    ...
    type=float,
    default=2.0,
    help="sigma parameter used for the weight map calculation [1]",
)
@click.option(
    "--w_0",
    type=float,
    default=2.0,
    help="w_0 parameter used for the weight map calculation [1]",
)
@click.option(
    "--w_c0",
    type=float,
...
@click.option(
    ...
    type=int,
    default=32,
    help="The batch size of the data sets for training.",
)
@click.option(
    "--shuffle_buffer",
    type=int,
...
@click.option(
    "--batch_size", type=int, default=2, help="Batch size used for the training."
)
@click.option(
    "--epochs", type=int, default=50, help="Number of epochs used for the training."
)
@click.option(
    "--iou_threshold",
    type=float,
...
def main(**kwargs):
    ...
    tf_pipe = TFPipeFamilyMachine(
        paths=kwargs["train_files"],
        ...
        delta_gamma=kwargs["delta_gamma"],
        delta_gain=kwargs["delta_gain"],
        delta_brightness=kwargs["delta_brightness"],
        lower_contrast=kwargs["lower_contrast"],
        upper_contrast=kwargs["upper_contrast"],
        rescale=kwargs["rescale"],
        n_repeats=kwargs["n_repeats"],
        train_seed=kwargs["train_seed"],
        val_seed=kwargs["val_seed"],
