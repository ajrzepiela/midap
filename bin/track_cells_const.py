...
img_names_sort = np.sort(
    glob.glob(images_folder + '*frame*'))[:int(args.num_frames)]
seg_names_sort = np.sort(
    glob.glob(segmentation_folder + '*frame*'))[:int(args.num_frames)]
...
# Process
tr = Tracking(img_names_sort, seg_names_sort,
              model_file, input_size, target_size)
tr.track_all_frames_const()

# Reduce results file for storage
results_all_red = np.empty(
...
# Save data
np.savez(output_folder + 'inputs_all_red_const.npz',
         inputs_all=np.array(tr.inputs_all))
np.savez(output_folder + 'results_all_red_const.npz',
