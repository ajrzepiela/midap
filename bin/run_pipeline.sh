...
tracking_family() {
  ...
  for i in $(seq $START $NUM_CHANNEL_TYPES); do
    CH="CHANNEL_$i"
    ${PYTHON_EXE} track_cells_crop.py --path "$PATH_FOLDER$POS/${!CH}/" --start_frame "$START_FRAME" --end_frame "$END_FRAME" --loglevel "${__VERBOSE}"
    ${PYTHON_EXE} generate_lineages.py --path "$PATH_FOLDER$POS/${!CH}/$TRACK_OUT_PATH" --loglevel "${__VERBOSE}"
  done
