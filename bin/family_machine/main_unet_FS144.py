...
# Load data
path = '../../data/unet/family_machine/FS144/'
proc = DataProcessor()

path_img_train = [path + 'train/img/pos33-mCherry_dual-A10200_cut.png']

path_mask_train = [path + 'train/mask/pos33-mCherry_dual-A10200_seg_2_new.png']
...
for i in range(len(path_img_train)):
    (X_train_tmp,
     ...
     ratio_cell_train_tmp,
     X_val_tmp,
     y_val_tmp,
     weight_maps_val_tmp,
     ratio_cell_val_tmp) = proc.run(path_img_train[i],
                                    path_mask_train[i])
...
model.fit(x=[X_train,
          ...
          callbacks=[callback],
          shuffle=True)
model.save_weights('../../model_weights/family_machine/model_weights_FS144.h5')
