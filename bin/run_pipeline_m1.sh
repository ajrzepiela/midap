...
if [[ $DATA_TYPE == "FAMILY_MACHINE" ]]
    then
        ...
        for POS in "${POS_UNIQ[@]}"; do
                ...
                then
                        echo $POS
                        $workflow_m1 restrict_frames.py                        
                        source settings.sh
                fi
			...
			# generate folder for tracking output (U-Net)
                ...
                then
                        ...
                        for i in $(seq 1 $NUM_CHANNEL_TYPES); do
                                ...
                                $workflow_m1 stack2frames.py --path $INP --pos $POS --channel /${!CH}/ --start_frame $START_FRAME --end_frame $END_FRAME --deconv $DECONVOLUTION
                        done
                fi
                ...
                then
                        ...
                        if [ -z "$CHANNEL_2" ] || [ -z "$CHANNEL_3" ]
                                then
                                $workflow_m1 frames2cuts.py --path_ch0 $PATH_FOLDER$POS/$CHANNEL_1/$RAW_IM
                                echo $PATH_FOLDER$POS$CHANNEL_1$RAW_IM
                        else
                                $workflow_m1 frames2cuts.py --path_ch0 $PATH_FOLDER$POS/$CHANNEL_1/$RAW_IM --path_ch1 $PATH_FOLDER$POS/$CHANNEL_2/$RAW_IM --path_ch2 $PATH_FOLDER$POS/$CHANNEL_3/$RAW_IM
                        fi
                fi
                ...
                then
                        ...
                        if [ "$PHASE_SEGMENTATION" == True ]
                                ...
                                for i in $(seq 1 $NUM_CHANNEL_TYPES); do
                                        CH="CHANNEL_$i"
                                        $workflow_m1 main_prediction.py --path_model_weights '../model_weights/model_weights_family_mother_machine/' --path_pos $PATH_FOLDER$POS --path_channel ${!CH} --postprocessing 1 --batch_mode 0
                                        $workflow_m1 analyse_segmentation.py --path_seg $PATH_FOLDER$POS/${!CH}/$SEG_IM_PATH/ --path_result $PATH_FOLDER$POS/${!CH}/
                                done
                ...
                #         for i in $(seq 1 $NUM_CHANNEL_TYPES); do
                #                 CH="CHANNEL_$i"
                #                 workflow_m1 seg2mat.py --path_cut $PATH_FOLDER$POS/${!CH}/$SEG_PATH$CUT_PATH --path_seg $PATH_FOLDER$POS/${!CH}/$SEG_IM_PATH --path_channel $PATH_FOLDER$POS/${!CH}/
                #         done
                # fi
	        ...
	        if [[ $RUN_OPTION == "BOTH" ]] || [[ $RUN_OPTION == "TRACKING" ]]
                then
                        ...
                        then 
                                for i in $(seq 1 $NUM_CHANNEL_TYPES); do
                                CH="CHANNEL_$i"
                                $workflow_m1 track_cells_crop.py --path $PATH_FOLDER$POS/${!CH}/ --start_frame $START_FRAME --end_frame $END_FRAME
                                $workflow_m1 generate_lineages.py --path $PATH_FOLDER$POS/${!CH}/$TRACK_OUT_PATH
                        done
                        elif [ "$PHASE_SEGMENTATION" == False ]
                        then
                        for i in $(seq 2 $NUM_CHANNEL_TYPES); do
...
if [[ $DATA_TYPE == "WELL" ]]
    then
        ...
        then
                echo "split frames"
                $workflow_m1 stack2frames.py --path $PATH_FILE_WO_EXT/$FILE_NAME --pos "" --channel "" --start_frame $START_FRAME --end_frame $END_FRAME --deconv $DECONVOLUTION
        ...
        then
                echo "segment images"
                $workflow_m1 main_prediction.py --path_model_weights '../model_weights/model_weights_well/' --path_pos $PATH_FILE_WO_EXT --path_channel "" --postprocessing 1
        fi
        ...
        then
                echo "run file-conversion"
                $workflow_m1 seg2mat.py --path_cut $PATH_FILE_WO_EXT/$SEG_PATH$CUT_PATH --path_seg $PATH_FILE_WO_EXT/$SEG_IM_PATH --path_channel $PATH_FILE_WO_EXT/
        fi
