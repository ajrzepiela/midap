    ...
    def run_image_stack(self, path_pos, path_cut, path_seg, path_seg_track, model_weights):
        ...
        if model_weights == 'watershed':
            ...
            for i, y in enumerate(y_preds):
                ...
                io.imsave(path_pos + path_seg + path_imgs[i].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_seg.png', seg_label.astype(int), check_contrast=False)

            io.imsave(path_pos + path_seg_track + path_imgs[0].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_full_stack_cut.tiff', np.array(imgs_pad).astype(float), check_contrast=False)
            io.imsave(path_pos + path_seg_track + path_imgs[0].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_full_stack_seg_bin.tiff', np.array(segs), check_contrast=False)
            io.imsave(path_pos + path_seg_track + path_imgs[0].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_full_stack_seg_prob.tiff', y_preds.astype(float), check_contrast=False)
