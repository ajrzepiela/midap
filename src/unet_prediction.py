...
class SegmentationPredictor():
    ...
    def select_weights(self, path_pos, path_cut, path_seg):
        ...
        list_files = np.sort(os.listdir(os.path.join(path_pos, path_cut)))
        ix_half = int(len(list_files) / 2)
        path_img = list_files[ix_half]

        img = self.scale_pixel_vals(io.imread(os.path.join(path_pos, path_cut, path_img)))
    ...
    def run_single_image(self, path_pos, path_cut, path_seg, path_img):
        ...
        io.imsave(os.path.join(path_pos, path_seg, path_img.replace('_cut.tif', '').replace('.tif', '') + '_seg.png'),
    ...
    def run_image_stack(self, path_pos, path_cut, path_seg, path_seg_track, model_weights):
        path_imgs = np.sort(os.listdir(os.path.join(path_pos, path_cut)))
        ...
        if model_weights == 'watershed':
            ...
            cuts = []
            for p in path_imgs:
                img = self.scale_pixel_vals(io.imread(os.path.join(path_pos, path_cut, p)))
                ...
                io.imsave(os.path.join(path_pos, path_seg,
                                       p.replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_seg.png'),
                          seg_label.astype('uint16'), check_contrast=False)
            fname = path_imgs[0].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + \
                    '_full_stack_seg_bin.png'
            io.imsave(os.patj.join(path_pos, path_seg_track, fname), np.array(segs), check_contrast=False)
            ...
            for i, y in enumerate(y_preds):
                ...
                io.imsave(os.path.join(path_pos, path_seg,
                                       path_imgs[i].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_seg.png'),
                    seg_label.astype(int), check_contrast=False)

            io.imsave(os.path.join(path_pos, path_seg_track,
                                   path_imgs[0].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_full_stack_cut.tiff'),
                      np.array(imgs_pad).astype(float), check_contrast=False)
            io.imsave(os.path.join(path_pos, path_seg_track,
                                   path_imgs[0].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_full_stack_seg_bin.tiff'),
                      np.array(segs), check_contrast=False)
            io.imsave(os.path.join(path_pos, path_seg_track,
                                   path_imgs[0].replace('_cut.tif', '').replace('_cut.png', '').replace('.tif', '') + '_full_stack_seg_prob.tiff'),
    ...
    def pad_image(self, img):
        ...
        img_pad[self.row_shape:, :self.col_shape] = img[(img.shape[0] - (img_pad.shape[0] - img.shape[0])):, :][
                                                    ::-1]  # pad mirrored image
        img_pad[:self.row_shape, self.col_shape:] = np.fliplr(
            np.flipud(img[:, (img.shape[1] - (img_pad.shape[1] - img.shape[1])):][::-1]))  # pad mirrored image
